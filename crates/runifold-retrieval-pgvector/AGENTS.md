...
# runifold-retrieval-pgvector Agent guide
...
Does not own: Agent conversation persistence.
...
Extend postgresql vector retrieval adapter within this crate; consult current public exports before adding another abstraction.
...
```sh
cargo test -p runifold-retrieval-pgvector --locked
cargo clippy -p runifold-retrieval-pgvector --all-targets --all-features --locked -- -D warnings
```
