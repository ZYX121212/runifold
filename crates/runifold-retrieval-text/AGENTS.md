...
# runifold-retrieval-text Agent guide
...
Does not own: Model execution and remote vector stores.
...
Extend local text retrieval adapter within this crate; consult current public exports before adding another abstraction.
...
```sh
cargo test -p runifold-retrieval-text --locked
cargo clippy -p runifold-retrieval-text --all-targets --all-features --locked -- -D warnings
```
