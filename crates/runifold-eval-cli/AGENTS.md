...
Does not own: Application Agent policy.
...
Extend evaluation cli and ci regression gates within this crate; consult current public exports before adding another abstraction.
...
```sh
cargo test -p runifold-eval-cli --locked
cargo clippy -p runifold-eval-cli --all-targets --all-features --locked -- -D warnings
```
