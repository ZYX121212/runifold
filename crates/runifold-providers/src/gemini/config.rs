//! Validated Gemini client configuration.
...
pub enum GeminiConfigError {
    ...
    #[error("Gemini embedding model cannot be empty")]
    EmptyEmbeddingModel,
...
impl GeminiConfig {
    ...
    /// Creates credential-free configuration for an application-controlled gateway.
    ///
    /// The gateway must add the upstream Gemini credential after authenticating
    /// the application. Browser and edge artifacts therefore contain no Google
    /// API key.
    ///
    /// # Errors
    ///
    /// Returns an error for an invalid or non-HTTP(S) base URL.
    pub fn gateway(base_url: &str) -> Result<Self, GeminiConfigError> {
        Self {
            api_key: None,
            base_url: Url::parse("https://generativelanguage.googleapis.com/v1beta/")?,
        }
    ...
    pub(crate) fn embedding_endpoint_url(&self, model: &str) -> Result<Url, url::ParseError> {
        let model = model.strip_prefix("models/").unwrap_or(model);
        self.base_url
            .join(&format!("models/{model}:batchEmbedContents"))
    }
}
...
mod tests {
    ...
    #[test]
    fn gateway_configuration_omits_the_upstream_credential() {
        let config = GeminiConfig::gateway("https://gateway.example/gemini/v1beta").unwrap();

        assert!(config.api_key.is_none());
        assert_eq!(
            config.endpoint_url("gemini-test").unwrap().as_str(),
            "https://gateway.example/gemini/v1beta/models/gemini-test:streamGenerateContent?alt=sse"
        );
    }
}
