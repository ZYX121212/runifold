//! Validated Anthropic client configuration.
...
impl AnthropicConfig {
    ...
    ///
    /// The gateway is responsible for authenticating the application and adding
    /// the upstream Anthropic credential. This constructor prevents a long-lived
    /// Anthropic API key from being embedded in browser or edge artifacts.
    ///
    /// # Errors
    ///
    /// Returns an error for an invalid, non-HTTP(S), or non-base URL.
    pub fn gateway(base_url: &str) -> Result<Self, AnthropicConfigError> {
        Self {
            api_key: None,
            base_url: Url::parse("https://api.anthropic.com/v1/")?,
            api_version: "2023-06-01".into(),
...
mod tests {
    ...
    fn gateway_configuration_omits_the_upstream_credential() {
        let config = AnthropicConfig::gateway("https://gateway.example/anthropic/v1").unwrap();
