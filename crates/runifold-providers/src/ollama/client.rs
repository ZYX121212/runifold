...
use super::{
    OllamaChunkDecoder, OllamaConfig, OllamaConfigError, OllamaEmbeddingModel, encode_request,
};
...
impl OllamaClient {
    ...
    ///
    /// Use [`Self::new`] for remote endpoints or bearer authentication.
    ///
    /// # Errors
    ///
    /// Returns [`OllamaConfigError`] when the built-in local endpoint cannot
    /// be constructed.
    pub fn local() -> Result<Self, OllamaConfigError> {
        OllamaConfig::local().map(Self::new)
    }
    ...
    /// Binds this transport and endpoint to one Ollama embedding model.
    ///
    /// # Errors
    ///
    /// Returns [`OllamaConfigError::EmptyEmbeddingModel`] for a blank model.
    pub fn embedding_model(
        &self,
        model: impl Into<String>,
    ) -> Result<OllamaEmbeddingModel, OllamaConfigError> {
        OllamaEmbeddingModel::new(self.config.clone(), self.http.clone(), model)
    }
}
...
impl ProviderModel for OllamaClient {
    ...
    Box::pin(async_stream::try_stream! {
        ...
        loop {
            ...
            match next {
                ...
                Some(Err(error)) => Err(transport(&error, deadline))?,
                None => {
