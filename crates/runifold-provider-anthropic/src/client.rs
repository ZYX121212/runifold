...
use crate::{AnthropicConfig, AnthropicEventDecoder, encode_request};
...
impl Model for AnthropicClient {
    ...
    fn stream(
        ...
        Box::pin(async move {
            ...
            let response =
                send_request(self.request_builder(&body, &context), &cancellation).await?;
            let status = response.status();
            let request_id = request_id(&response);
            if !status.is_success() {
                let retry_after = retry_after(&response);
                let payload = read_error_body(response, &cancellation).await?;
                return Err(http_error(status, request_id, &payload, retry_after));
            }
            Ok(event_stream(response, request_id, cancellation, warnings))
        })
    }
}
