...
# runifold-store-sqlite Agent guide
...
Does not own: A separate execution model or implicit unsafe recovery.
...
Extend sqlite implementations of durable runtime stores within this crate; consult current public exports before adding another abstraction.
...
```sh
cargo test -p runifold-store-sqlite --features bundled --locked
cargo clippy -p runifold-store-sqlite --all-targets --all-features --locked -- -D warnings
```
